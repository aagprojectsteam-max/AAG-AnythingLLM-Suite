#!/usr/bin/env python3
"""On-demand local bridge for the separately versioned Scene Identity contract."""
...
REQUEST_KEYS = {
    ...
    "scene_contract_sha256", "scene_profile", "prompt_sha256", "release", "route", "lease_token",
    "caller", "submitted_at",
}
...
def load_config() -> dict:
    ...
    if config.get("release") != RELEASE or config.get("scene_contract_sha256") != CONTRACT_SHA:
        raise BridgeFailure("SCENE_IDENTITY_CONTRACT_INTEGRITY_FAILURE", "Scene Identity production configuration drifted.")
    if sha256(CONTRACT_PATH) != CONTRACT_SHA:
        raise BridgeFailure("SCENE_IDENTITY_CONTRACT_INTEGRITY_FAILURE", "Scene Identity contract hash mismatch.")
    return config


def validate_message(value: dict, config: dict) -> dict:
    if set(value) != REQUEST_KEYS or value.get("schema_version") != "aag.human-identity.scene.bridge-request.v1":
        raise BridgeFailure("REQUEST_INVALID", "The local Scene Identity request schema is invalid.")
    ...
    if not isinstance(prompt, str) or not 20 <= len(prompt) <= 700 or len(prompt.split()) > 70 or any(ord(character) < 32 and character not in "\n\t" for character in prompt):
        raise BridgeFailure("SCENE_IDENTITY_PROMPT_INVALID", "The bounded scene prompt is invalid or exceeds the Scene Identity envelope.")
    if not hmac.compare_digest(hashlib.sha256(prompt.encode()).hexdigest(), str(value.get("prompt_sha256", ""))):
        raise BridgeFailure("SCENE_IDENTITY_PROMPT_INTEGRITY_FAILURE", "The scene prompt hash does not match the trusted request.")
    if value.get("contract_id") != "structured-scene-c" or value.get("scene_contract_sha256") != CONTRACT_SHA or value.get("release") != RELEASE:
        raise BridgeFailure("SCENE_IDENTITY_CONTRACT_INTEGRITY_FAILURE", "The request does not match the active Scene Identity contract.")
    profile = config.get("supported_profiles", {}).get(str(value.get("scene_profile", "")))
    if not profile or value.get("width") != profile.get("width") or value.get("height") != profile.get("height"):
        raise BridgeFailure("SCENE_IDENTITY_FRAMING_UNSUPPORTED", "The request is outside the bounded Scene Identity framing profiles.")
    if not isinstance(value.get("seed"), int) or not 0 <= value["seed"] <= 0xFFFFFFFF:
        raise BridgeFailure("SCENE_IDENTITY_SEED_INVALID", "The Scene Identity seed is invalid.")
...
def error_response(request_id: str, code: str, message: str, retryable: bool = False,
    ...
    response = {
        "schema_version": "aag.human-identity.scene.response.v1", "request_id": request_id,
        "release": RELEASE, "contract_id": "structured-scene-c", "scene_contract_sha256": CONTRACT_SHA,
        "status": "CANCELLED" if code == "JOB_CANCELLED" else "FAIL",
...
def process_request(request_file: Path) -> None:
    ...
    try:
        ...
        atomic_json(worker_request, {
            "schema_version": "aag.human-identity.scene.request.v1", "request_id": request_id,
            ...
            "seed": message["seed"], "width": message["width"], "height": message["height"],
            "steps": 25, "guidance_scale": 7.0, "id_scale": 1.2,
            "contract_id": message["contract_id"], "scene_contract_sha256": message["scene_contract_sha256"],
            "scene_profile": message["scene_profile"], "prompt_sha256": message["prompt_sha256"],
        })
        ...
        verify_lease(message)
        worker = sandbox([PYTHON, str(RUNTIME / "runtime/scene_worker.py"), "--request", str(worker_request), "--report", str(worker_report), "--output", str(worker_output), "--cancel-file", str(cancel), "--stage-file", str(stage_file)])
        ...
        if gate_result.returncode or evaluation.get("status") != "PASS":
            code = str(evaluation.get("error_code") or "SCENE_IDENTITY_QUALITY_REJECTED")
            raise BridgeFailure(code, "The generated scene was rejected by the locked Scene Identity quality contract.")
        ...
        response = {
            "schema_version": "aag.human-identity.scene.response.v1", "request_id": request_id,
            "release": RELEASE, "scene_contract_sha256": CONTRACT_SHA,
            "contract_id": message["contract_id"], "scene_profile": message["scene_profile"],
            "prompt_sha256": message["prompt_sha256"], "status": "PASS",
            ...
            "evaluation": {
                key: evaluation.get(key) for key in (
                    ...
                    "face_evaluable", "structural_scene_result", "manual_visual_review_required",
                    "automatic_scene_fidelity_scope",
                )
            },
