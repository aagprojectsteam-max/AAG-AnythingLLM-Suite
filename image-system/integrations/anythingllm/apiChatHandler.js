...
const {
  isAagLocalImageCommand,
  executeAagLocalImageCommand,
} = require("./commands/aagIdentity");
const {
  isAagOrdinaryCommand,
  executeAagOrdinaryCommand,
} = require("./commands/aagOrdinary");
/**
...
async function chatSync({
  ...
  // AAG Human Identity attachments must never enter a remote LLM request.
  // The explicit local command executes the deployed provider inside the
  // AnythingLLM boundary before agent or connector construction.
  if (isAagLocalImageCommand(message)) {
    return executeAagLocalImageCommand(
      workspace,
      message,
      uuid,
      user,
  ...
  // Unambiguous ordinary, upscale, subject-transform, and identity-preserve
  // intents execute locally before connector construction. This prevents the
  // image prompt or attachment from reaching the workspace's remote LLM.
  if (isAagOrdinaryCommand(message, attachments)) {
    return executeAagOrdinaryCommand(
      workspace, message, uuid, user, thread, null, attachments, null,
      { apiSessionId: sessionId }
    );
  }
...
async function streamChat({
  ...
  // Keep local Human Identity reference bytes out of agent/LLM construction.
  if (isAagLocalImageCommand(message)) {
    const data = await executeAagLocalImageCommand(
      workspace,
      message,
      uuid,
      user,
  ...
  if (isAagOrdinaryCommand(message, attachments)) {
    const data = await executeAagOrdinaryCommand(
      workspace, message, uuid, user, thread, response, attachments, null,
      { apiSessionId: sessionId }
    );
    writeResponseChunk(response, data);
    return;
