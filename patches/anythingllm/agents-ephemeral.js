...
class EphemeralAgentHandler extends AgentHandler {
  ...
  async #attachPlugins(args) {
    for (const name of this.#funcsToLoad) {
      ...
      if (name.startsWith("@@")) {
        ...
        callOpts.AAG_WORKSPACE_ID = this.#workspace?.id ?? null;
        callOpts.AAG_THREAD_ID = this.#threadId ?? null;
        callOpts.AAG_USER_ID = this.#userId ?? null;
        callOpts.AAG_INVOCATION_UUID = this.#invocationUUID;
        // One EphemeralAgentHandler instance represents exactly one trusted
        // user turn, so its invocation UUID is also its stable retry-safe turn.
        callOpts.AAG_TURN_ID = this.#invocationUUID;
  ...
  async createAIbitat(
    ...
    // Keep API, scheduled, Telegram, and one-off Agent executions under the
    // same pre-rerank and pre-execution workspace policy as websocket turns.
    installWorkspaceToolPolicy({
    ...
    // Keep API/one-off Agent execution aligned with the websocket path.
    installAagArtifactPresentation({
      aibitat: this.aibitat,
      generatedImagesPath,
      log: (message) => this.log(message),
