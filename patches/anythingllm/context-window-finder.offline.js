...
class ContextWindowFinder {
  ...
  static remoteRefreshDisabled =
    process.env.AAG_DISABLE_REMOTE_MODEL_MAP_REFRESH === "true";
  static remoteUrl =
  ...
  constructor() {
    ...
    // Local/offline deployments may intentionally retain the bundled/cached map
    // and prohibit a background GitHub fetch during application boot.
    if (ContextWindowFinder.remoteRefreshDisabled) {
      this.log(
        "Remote model map refresh disabled by AAG_DISABLE_REMOTE_MODEL_MAP_REFRESH; using local cached/legacy data"
      );
    } else if (this.isCacheStale || !fs.existsSync(this.cacheFilePath)) {
